...
setup:
	...
	MOV checkloop, #0; If checkloop is zero, new digit can be read.
...
start:
	...
	;Hence we can move zero in checkloop.
	MOV checkloop,#0;
...
;	JMP $			; program execution arrives here when key is found - do nothing
	...
	MOV checkloop, #1; The key is found, now, unless the button is left open, we will not read next digit.
	CJNE R0, #24h,start; If the key is larger, then jump.
	;4 digits are taken in. 
	MOV R1, #20h; The numbers begin from here
	num_convert: 
		MOV A,@R1;
		MOVC A,@A+DPTR; Convert the keypad digits to actual keys of the keypad.
		
		CLR C; 
		SUBB A, #30h; 0 digit is 30. Subtracting 30h gives the numerical value of digit.
		...
		MOV B, R0;
		;INC B; 
		CJNE A, B, num_convert;
	;The digits are now converted, they can be added and transmitted
	...
	MOV @R1, A;
	CLR C;
	SUBB A, #10; Carry is set if number is smaller than 10.
	JNC fix_bits;
	after_fix:
		MOV R1,#22h;
		MOV A,@R1;
		MOV R1,#20h;
		CLR C; Same number larger by one problem occured
		ADDC A,@R1;
		MOV @R1, A;

	JMP Tx; Now that the sum is set, we are good to go to transmit
	
	fix_bits:
		MOV @R1,A; 
		DEC R1;
		INC @R1; At most one can be added in case of addition of two digits.
		JMP after_fix
...
Tx: ; The complete sending and setup of transmission
	...
	MOV R1, #20h;
	MOV R0, #22h; We only need to transmit the first two bits
	;Run this on Even Parity
	Tx_loop:
		MOV A,@R1;
		;MOVC A,@A+DPTR; Convert the keypad digits to actual keys of the keypad.
		CLR C; As in the above case with SUBB, for some reason the answer output is 1 number larger in the second time
		ADDC A, #30h; This gives us the ASCII equivalent of digits.
		MOV SBUF, A;
		INC R1;
		MOV A, R1;
		MOV B, R0;
...
Rx:
	...
	Rx_loop:
		...
		MOV @R1, A;
		INC R1;
		CJNE R1, #32H,Rx_loop; 8 Digits for DOB.
